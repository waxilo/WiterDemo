...
- **单活跃会话**：同一账号多端登录时，任一端成功写入后自动下线其他所有端（写入前校验会话有效性，被踢端立即无法再写；读操作不受影响）
...
- 单会话抢占：AT 携带会话 id（`sid`），写操作前置校验会话有效 + 成功后吊销同账号其他会话（`revokeAllExcept`）；被踢端 401「账号已在其他设备使用，请重新登录」自动登出
